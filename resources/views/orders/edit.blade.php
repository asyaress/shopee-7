...
@section('title', 'Edit Pesanan - Toedjoe Order System')
...
<div class="report-shell hub-form-page">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="order_number"
                                    class="hub-form-control @error('order_number') is-invalid @enderror"
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="order_date"
                                    class="hub-form-control @error('order_date') is-invalid @enderror"
    ...
    <div class="col-md-6 mb-3">
        ...
        <div class="input-group">
            <select name="customer_id" id="customer_id"
                class="hub-form-control @error('customer_id') is-invalid @enderror"
                required>
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="date" name="completion_date"
            class="hub-form-control @error('completion_date') is-invalid @enderror"
            value="{{ old('completion_date', $order->completion_date->format('Y-m-d')) }}" required>
        ...
        @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select name="jenis_pengiriman" class="hub-form-control @error('jenis_pengiriman') is-invalid @enderror" required>
                                <option value="">Pilih Jenis Pengiriman</option>
                                <option value="Gosend" {{ old('jenis_pengiriman', $order->jenis_pengiriman) == 'Gosend' ? 'selected' : '' }}>Gosend</option>
                                <option value="JNE" {{ old('jenis_pengiriman', $order->jenis_pengiriman) == 'JNE' ? 'selected' : '' }}>JNE</option>
                                <option value="SPX" {{ old('jenis_pengiriman', $order->jenis_pengiriman) == 'SPX' ? 'selected' : '' }}>SPX</option>
                                <option value="Anter Aja" {{ old('jenis_pengiriman', $order->jenis_pengiriman) == 'Anter Aja' ? 'selected' : '' }}>Anter Aja</option>
                                <option value="POS" {{ old('jenis_pengiriman', $order->jenis_pengiriman) == 'POS' ? 'selected' : '' }}>POS</option>
                                <option value="JTR" {{ old('jenis_pengiriman', $order->jenis_pengiriman) == 'JTR' ? 'selected' : '' }}>JTR</option>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="jenis_transaksi" class="hub-form-control @error('jenis_transaksi') is-invalid @enderror" required>
                                <option value="">Pilih Jenis Transaksi</option>
                                <option value="Shopee" {{ old('jenis_transaksi', $order->jenis_transaksi) == 'Shopee' ? 'selected' : '' }}>Shopee</option>
                                <option value="Website" {{ old('jenis_transaksi', $order->jenis_transaksi) == 'Website' ? 'selected' : '' }}>Website</option>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" class="hub-form-control @error('status') is-invalid @enderror" required>
                                <option value="pending" {{ old('status', $order->status) == 'pending' ? 'selected' : '' }}>Menunggu</option>
                                <option value="in_progress" {{ old('status', $order->status) == 'in_progress' ? 'selected' : '' }}>Sedang Proses</option>
                        ...
                        <div class="mb-3">
                            ...
                            <table class="table table-bordered align-middle" id="productTable">
                                ...
                                <tbody>
                                    @php $productsList = old('product_id', collect($order->orderItems)->pluck('product_id')->toArray()); @endphp
                                    ...
                                    <tr>
                                            ...
                                            <select name="product_id[]" class="hub-form-control product-select" required>
                                                ...
                                                @foreach($products as $product)
                                                    <option value="{{ $product->id }}"
                                                        ...
                                                        {{ old('product_id.' . $i, $order->orderItems[$i]->product_id ?? '') == $product->id ? 'selected' : '' }}>
                                        ...
                                        <td>
                                            <input type="text" name="product_name[]" class="hub-form-control product-name"
                                                ...
                                                value="{{ old('product_name.' . $i, $order->orderItems[$i]->product_name ?? '') }}" required>
                                        </td>
                                        <td>
                                            <input type="number" name="quantity[]" class="hub-form-control quantity" value="{{ old('quantity.' . $i, $order->orderItems[$i]->quantity ?? 1) }}" min="1" required>
                                        </td>
                                        <td>
                                            <input type="number" name="price[]" class="hub-form-control price" value="{{ old('price.' . $i, $order->orderItems[$i]->price ?? 0) }}" min="0" step="100" required>
                                        </td>
                                        <td>
                                            <input type="text" class="hub-form-control subtotal"
                                            value="{{ old('quantity.' . $i, $order->orderItems[$i]->quantity ?? 1) * old('price.' . $i, $order->orderItems[$i]->price ?? 0) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="notes" class="hub-form-control @error('notes') is-invalid @enderror"
                                rows="3" placeholder="Tambahkan catatan khusus untuk pesanan ini...">{{ old('notes', $order->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="hub-card">
                    <div class="card-body">
                        ...
                        <a href="{{ route('orders.index') }}" class="hub-btn hub-btn-outline w-100">
